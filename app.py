...
if st.session_state.step == 2:
    ...
    current_prompt = st.text_area("🔍 **Current Prompt** (edit if needed):\n", value=current_prompt, key=prompt_key, height=400)
    ...
    for idx, idea in enumerate(all_ideas):
        ...
        edited_ideas.append({"label": f"{micro} — {expanded}", "type": idea["type"], "data": {"micro_core_idea": micro, "expanded_core_idea": expanded}})
    ...
    for idx, whw in enumerate(all_whw):
        ...
        edited_whws.append({"label": f"{why} — {how} — {what}", "type": whw["type"], "data": {"why": why, "how": how, "what": what}})
    ...
    for idx, sol in enumerate(all_solutions):
        ...
        edited_solutions.append({"label": f"{problem} — {solution}", "type": sol["type"], "data": {"problem": problem, "solution": solution}})
...
if st.session_state.step == 3:
    ...
    default_prompt = get_default_prompt_for_step(st.session_state.step)
    current_prompt = st.session_state.step_inputs.get(prompt_key, default_prompt)
    current_prompt = st.text_area("🧠 Edit Prompt for This Step", value=current_prompt, key=prompt_key, height=400)
    st.session_state.step_inputs[prompt_key] = current_prompt
    ...
    st.subheader("Cult Words – Alliterative")
